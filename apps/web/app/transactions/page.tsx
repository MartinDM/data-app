  ...
  return (
    <div className="rounded-lg border p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t">
        <span className={`px-2 py-1 rounded-full text-xs ${transaction.isOnline ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
...
export default function TransactionsPage() {
  ...
  const categories = ['all', ...Array.from(new Set(transactions.map(t => t.merchantCategory)))];

  return (
    <div className="container mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Transactions</h1>
        <p className="text-muted-foreground">View and manage your credit card transactions</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-card p-4 rounded-lg border">
          <h3 className="text-sm font-medium text-muted-foreground">Online vs In-Store</h3>
          <p className="text-2xl font-bold">
            {filteredTransactions.filter(t => t.isOnline).length} / {filteredTransactions.filter(t => !t.isOnline).length}
          </p>
        </div>
      </div>
